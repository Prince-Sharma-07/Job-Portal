...
export default function savedCard({job}) {
  ...
  return (
      ...
      <div className='flex flex-col gap-4 border-2 p-4 justify-between h-fit rounded-2xl'>
        ...
        <span className='font-medium'>Location <span>{job.job_location}</span></span>
        </div>
        <button onClick={()=>removeFromSave(job.id)} className='text-blue-400 hover:text-blue-600'>Remove from fav</button>
    </div>
